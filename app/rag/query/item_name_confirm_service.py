...
def get_history_messages(session_id:str, limit:int = 10) -> list[dict]:
    ...
    # 有效校验
    final_message_list = [item for item in history_message_list if not item["item_names"] and len(item['item_names']) > 0]
...
def call_llm_deal_data(history_text, original_query) -> dict:
    ...
    # 2. 构建提示词
    prompt_text = load_prompt("rewritten_query_and_itemnages", history_text=history_text, query=original_query)
    messages = [
    ...
    # 4. 执行获取结果  {item_names: [], rewritten_query: “重写问题”}
...
def query_item_name_milvus(item_names: list[str]) -> dict[str, list[dict]]:
    ...
    for item_name in item_names:
        # 3. 对每个item_name向量化，获取对应的稠密和稀疏向量
...
def select_item_names(milvus_result_dict):
    ...
    # 2. 循环处理每个item_name 对应的列表
    for item_name, milvus_list in milvus_result_dict.items():
        # 没必要
        milvus_list.sort(key=lambda x : x['score'], revserse=True)
        # 筛选列表  高分[> 0.7]  可选[0.6 ~ 0.7]
        high_item_names = [item['item_name'] for item in milvus_list if item['score'] > 0.7]
        op_item_names = [item['item_name'] for item in milvus_list if 0.6 < item['score'] <= 0.7]
        ...
        if len(high_item_names) > 0:
            ...
            continue
        # 没有确认的，从可选中获取
        if len(op_item_names) > 0:
            options_item_name_list.extend(op_item_names[:2])
            continue
...
def confirm_item_name(state: QueryGraphState) -> QueryGraphState:
    ...
    # 参数 history_text 和 original_query  响应： dict:    {item_name:[], rewritten_query: ''}
    result_dict = call_llm_deal_data(history_text, original_query)

    item_name_dict = {}

    # 5. 进行校验，如果没有 item_names 无需调用向量查询
    if len(result_dict['item_names']) > 0:
        milvus_result_dict = query_item_name_milvus(result_dict['item_names'])
